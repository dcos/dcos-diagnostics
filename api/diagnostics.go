...
func (j *DiagnosticsJob) getHTTPAddToZip(node Node, endpoints map[string]string, folder string, zipWriter *zip.Writer,
	...
	for fileName, httpEndpoint := range endpoints {
		...
		if resp.StatusCode != http.StatusOK {
			...
			resp.Body.Close()
			if err != nil {
				errMsg = fmt.Sprintf("unable to fetch %s. Return code %d. Could not read body: %s", fullURL, resp.StatusCode, err)
			} else {
		...
		e := resp.Body.Close()
		if e != nil {
			logrus.WithError(e).Warn("Unable to close response body")
		}
